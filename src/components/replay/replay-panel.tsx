...
/**
 * Resolves a custom property to a literal value.
 *
 * The chart tokens are declared as `var()` chains into the brand primitives, and a
 * canvas needs a real colour string. Browsers substitute `var()` in computed custom
 * properties, but not every engine does it at every nesting depth, so this follows
 * the chain itself rather than trusting it. No hex appears in this file as a result.
 */
...
export function ReplayPanel() {
  ...
  const draw = useCallback(() => {
    ...
    // Candles.
    const slot = plotWidth / VISIBLE;
    const bodyWidth = Math.max(1, Math.floor(slot * 0.62));
    const offset = VISIBLE - visible.length;
