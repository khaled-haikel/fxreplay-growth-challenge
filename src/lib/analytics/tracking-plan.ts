...
/** Where an event is allowed to originate. */
export type EmittedFrom = 'client' | 'server';

interface EventDefinition<S extends z.ZodTypeAny> {
  /** Why this event exists. Rendered into the generated docs. */
  description: string;
  /** The user or system action that fires it. */
  trigger: string;
  emittedFrom: EmittedFrom;
  properties: S;
}

function defineEvent<S extends z.ZodTypeAny>(def: EventDefinition<S>) {
  return def;
}
...
export const trackingPlan = {
  ...
  replay_started: defineEvent({
    description:
      'Visitor starts the interactive replay. First signal of intent beyond scrolling.',
  ...
  trade_closed: defineEvent({
    description:
      'Visitor closes a position and sees the result. Activation event: the point at ' +
  ...
  replay_completed: defineEvent({
    description:
      'Replay reached the end of its data without the visitor ever opening a trade. ' +
  ...
  signup_failed: defineEvent({
    description:
      'Signup attempt did not produce an account. Guardrail metric: if a variant ' +
  ...
  account_created: defineEvent({
    description:
      'Primary conversion. An account row was committed. Emitted server-side so the ' +
...
export type TrackingPlan = typeof trackingPlan;
export type EventName = keyof TrackingPlan;

export type EventProperties<E extends EventName> = z.infer<
  TrackingPlan[E]['properties']
>;

/** Full payload as sent: declared properties plus shared context. */
export type EventPayload<E extends EventName> = EventProperties<E> & EventContext;
...
] as const satisfies ReadonlyArray<{ step: number; event: EventName; label: string }>;
...
export const guardrails = [
  ...
  },
  {
    ...
    rule:
      'Must not decrease. If more people start the replay but fewer finish a trade, ' +
...
export function validateEvent<E extends EventName>(
  ...
  context: unknown,
  emittedFrom: EmittedFrom,
): { properties: EventProperties<E>; context: EventContext } {
  ...
  if (definition.emittedFrom !== emittedFrom) {
    throw new TrackingPlanViolation(
      String(event),
      `may only be emitted from the ${definition.emittedFrom}, attempted from the ${emittedFrom}`,
    );
  }
  ...
  if (!parsedProperties.success) {
    throw new TrackingPlanViolation(
      String(event),
      `invalid properties: ${parsedProperties.error.issues
  ...
  if (!parsedContext.success) {
    throw new TrackingPlanViolation(
      String(event),
      `invalid context: ${parsedContext.error.issues
        .map((i) => `${i.path.join('.') || '(root)'} ${i.message}`)
        .join('; ')}`,
    );
  }
  ...
  return {
    properties: parsedProperties.data as EventProperties<E>,
...
export const acceptedSystemEvents = [
  ...
  },
  {
    ...
    reason:
      'Emitted by PostHog alongside a flag evaluation that belongs to a running ' +
  ...
  },
  {
    event: '$web_vitals',
    reason:
      'Core Web Vitals, captured by posthog-js performance instrumentation. Low ' +
